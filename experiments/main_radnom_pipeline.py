...
DECOMPOSITION_FUNCTIONS = {
    "PCA": lambda x, ns: x.run_latent_pca(ns),
    "ICA": lambda x, ns: x.run_latent_ica(ns),
    "kmeans": lambda x, ns: x.run_latent_kmeans(ns),
    "AAHC": lambda x, ns: x.run_latent_aahc(ns),
    "TAAHC": lambda x, ns: x.run_latent_taahc(ns),
    "HMM": lambda x, ns: x.run_latent_hmm(ns),
}
...
def main(n_subjects, ts_length, n_channels, n_states, workers):
    ...
    for i in range(n_subjects):
        data = get_random_gaussian_data(n_channels, ts_length, SAMPLING_RATE)
